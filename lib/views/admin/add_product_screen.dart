...
class _AddProductScreenState extends State<AddProductScreen> {
  ...
  void initState() {
    ...
    if (widget.editing != null) {
      ...
      unitsData = UnifiedUnitsData(
        ...
        baseIsActive: _getBaseUnitIsActive(
          p.units,
        ), // استخراج baseIsActive من الوحدة الأساسية
        baseIsPrimary: true, // الوحدة الأساسية دائماً primary
        baseIsBestSeller: p.isBestSeller,
        additionalUnits:
            p.units?.where((unit) => !unit.isPrimary).toList() ?? [],
      );
    } else {
  ...
  bool _getBaseUnitIsActive(List<ProductUnit>? units) {
    if (units == null || units.isEmpty)
      return true; // افتراضياً متوفر إذا لم تكن هناك وحدات
                ...
                ProductFormSection(
                  ...
                  children: [
                    UnifiedUnitsManager(
                      ...
                      initialBaseIsActive:
                          unitsData?.baseIsActive, // إضافة معامل منفصل
