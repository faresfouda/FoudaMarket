...
class _SpecialOffersScreenState extends State<SpecialOffersScreen> {
  ...
  void initState() {
    ...
    // Load special offers
    _productBloc.add(const FetchSpecialOffers(limit: 20));
  ...
  void _loadMoreOffers() {
    if (!_isLoadingMore && _productBloc.state is SpecialOffersLoaded) {
      final currentState = _productBloc.state as SpecialOffersLoaded;
      if (currentState.products.length >= 20) {
        // Load more products
        _productBloc.add(const FetchSpecialOffers(limit: 40));
      }
    }
  }
                      ...
                      listener: (context, state) {
                        if (state is ProductsError) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text('خطأ في تحميل العروض: ${state.message}'),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            'لا توجد عروض خاصة متاحة حالياً',
            style: TextStyle(
              fontSize: 14,
